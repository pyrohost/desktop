...
var modifyFilename$1 = function modifyFilename(pth, modifier) {
  ...
  var ext = path$1.extname(pth);
  return path$1.join(path$1.dirname(pth), modifier(path$1.basename(pth, ext), ext));
};
const pathExists = pathExistsExports;
const modifyFilename2 = modifyFilename$1;
const incrementer = (filePath) => {
  let counter = 0;
  return () => modifyFilename2(filePath, (filename, extension) => `${filename} (${++counter})${extension}`);
};
...
var unusedFilenameExports = unusedFilename$2.exports;
var escapeGoat = {};
(function(exports) {
...
const { app, BrowserWindow, shell, dialog } = require$$0$3;
const unusedFilename = unusedFilenameExports;
const pupa = pupa$1;
const extName = extNameExports;
...
electronDl.exports.download = (window_, url, options) => new Promise((resolve, reject) => {
  options = {
    ...options,
...
var electronDlExports = electronDl.exports;
const electron$1 = require$$0$3;
...
const { download } = electronDlExports;
  ...
  const handleContextMenu = (event, props) => {
    ...
    const defaultActions = {
      ...
      saveImageAs: decorateMenuItem({
        ...
        click(menuItem) {
          props.srcURL = menuItem.transform ? menuItem.transform(props.srcURL) : props.srcURL;
          download(win2, props.srcURL, { saveAs: true });
        }
      }),
      ...
      saveLinkAs: decorateMenuItem({
        ...
        click(menuItem) {
          props.linkURL = menuItem.transform ? menuItem.transform(props.linkURL) : props.linkURL;
          download(win2, props.linkURL, { saveAs: true });
        }
      }),
      ...
      inspect: () => ({
        ...
        click() {
          webContents(win2).inspectElement(props.x, props.y);
    ...
    if (options.menu) {
      menuTemplate = options.menu(defaultActions, props, win2, dictionarySuggestions, event);
    }
    if (options.prepend) {
      const result = options.prepend(defaultActions, props, win2, event);
      if (Array.isArray(result)) {
        menuTemplate.unshift(...result);
      }
    }
    if (options.append) {
      const result = options.append(defaultActions, props, win2, event);
      if (Array.isArray(result)) {
...
const contextMenu = /* @__PURE__ */ getDefaultExportFromCjs(electronContextMenu);
process.env.DIST = path$2.join(__dirname, "../dist");
process.env.VITE_PUBLIC = require$$0$3.app.isPackaged ? process.env.DIST : path$2.join(process.env.DIST, "../public");
let win;
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
function createWindow() {
  win = new require$$0$3.BrowserWindow({
    icon: path$2.join(process.env.VITE_PUBLIC, "icon.png"),
  ...
  const view = new require$$0$3.BrowserView();
...
require$$0$3.app.on("activate", () => {
  if (require$$0$3.BrowserWindow.getAllWindows().length === 0) {
    createWindow();
  }
});
require$$0$3.app.whenReady().then(createWindow);
